...
from collections import deque  # NEW: For history buffers
...
class SlicingModelHandler:
    def __init__(self, model_path: str,
                 ...
                 hysteresis_window: int = 3,  # NEW: Moving average window size
                 threshold_throughput_delta: float = 10000000.0,  # NEW: 10 Mbps
                 min_decision_interval: float = 4.0):  # NEW: Seconds between full decisions
        ...
        # NEW: History buffers for hysteresis (store [slice1, slice2] lists)
        self.dl_history = deque(maxlen=hysteresis_window)
        self.ul_history = deque(maxlen=hysteresis_window)
        
        # NEW: Tracking for thresholds and rate limiting
        self.last_kpis = None
    ...
    # NEW: Check if thresholds are met (significant KPI change)
    ...
    # NEW: Apply hysteresis (moving average) to smooth allocations
    ...
    def predict(self, kpi_data: Dict) -> Dict:
        ...
        # NEW: Rate limiting - return last if too soon
        if current_time - self.last_decision_time < self.min_decision_interval:
            print(f"DEBUG: Rate limit hit; returning last allocations: {self.last_allocations}")
            return self.last_allocations or {"error": "No prior allocations available; request ignored due to rate limit"}
        
        # NEW: Threshold check - return last if no significant change
        if not self._check_thresholds(kpi_data):
            print(f"DEBUG: Thresholds not met; returning last allocations: {self.last_allocations}")
            return self.last_allocations or {"error": "No significant change detected; reusing prior allocations"}
        ...
        with torch.no_grad():  # Disable gradients for speed and safety
            ...
            # NEW: Apply hysteresis to raw outputs
...
try:
    ...
    model_handler = SlicingModelHandler(model_path=MODEL_PATH)  # NEW: Pass tunables here if customizing
