...
class SlicingModelHandler:
    def __init__(self, model_path: str):
        ...
        # Define the flexible portion, which MUST match the training environment
        ...
        # Use map_location to ensure it works on a CPU-only server
        self.model.load_state_dict(torch.load(model_path, map_location=torch.device('cpu')))
        
        # **CRITICAL:** Set the model to evaluation mode for consistent, deterministic inference.
        self.model.eval()
        print(f"✅ Nuance-Only Model '{model_path}' loaded successfully.")
    ...
    def predict(self, kpi_data: Dict) -> Dict:
        """
        Takes raw KPI data and returns a nuanced, proportional allocation by
        applying the same action space transformation used in training.
        """
        with torch.no_grad(): # Disable gradients for speed and safety
            ...
            # Get the agent's raw decision on how to split the flexible portion
            agent_decision_dl = alpha_dl / alpha_dl.sum()
            agent_decision_ul = alpha_ul / alpha_ul.sum()

            # Apply the action space transformation to get the final allocation
            flexible_alloc_dl = agent_decision_dl * self.flexible_resource
            flexible_alloc_ul = agent_decision_ul * self.flexible_resource
            
            final_action_dl = flexible_alloc_dl + self.min_allocation
            final_action_ul = flexible_alloc_ul + self.min_allocation
            
            # The safety net/clamp logic is no longer needed. The output is always valid.

            # Format the simple response as requested
            response = {
                "downlink_allocation": {
                    "slice1": round(final_action_dl[0].item(), 4),
                    "slice2": round(final_action_dl[1].item(), 4)
                },
                "uplink_allocation": {
                    "slice1": round(final_action_ul[0].item(), 4),
                    "slice2": round(final_action_ul[1].item(), 4)
                }
            }
            return response
...
CHAMPION_MODEL_FILENAME = 'nuanced_allocator_seed1.pth' 
